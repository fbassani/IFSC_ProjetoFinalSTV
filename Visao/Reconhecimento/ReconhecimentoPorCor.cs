    ...
    public class ReconhecimentoPorCor : IEstrategiaDeReconhecimento {
        ...
        private int VerifiqueCor(IplImage origem) {
            ...
            using (canais[2] = canais[2].Clone()) {
                var ajusteCanal1 = Task.Run(() => AjusteIntencidadeDeAcordoComTolerancia(canais[0], _b));
                var ajusteCanal2 = Task.Run(() => AjusteIntencidadeDeAcordoComTolerancia(canais[1], _g));
                var ajusteCanal3 = Task.Run(() => AjusteIntencidadeDeAcordoComTolerancia(canais[2], _r));
        ...
        private static void AjusteIntencidadeDeAcordoComTolerancia(IplImage imagem, CorTolerancia corTolerancia) {
            var height = imagem.Height;
            var width = imagem.Width;
            var widthStep = imagem.WidthStep;
